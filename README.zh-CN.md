...
或者在[列表页面](https://udonite.github.io/vpm/)上点击 **Add to VCC**。

需要 VRChat Worlds SDK 3.10 或更高版本，以及 Unity 2022.3。除此之外不需要安装任何东西：编译器依赖的库已经打包在内。
...
Udon 有一些真实存在的限制：没有异常、没有可变的静态字段、没有 `Awake`、没有 tag。Udonite 会明确地拒绝这些写法。每一次拒绝都是控制台里的一行，带一个 `UDN` 代码，说明拒绝了什么、以及应该改写成什么；不会有任何东西被悄悄丢掉。被拒绝的 behaviour 不会生成 Udon 程序，项目里其余的部分照常编译。详见[语言支持](docs/language-support.md)（英文）。
...
- [网络](docs/networking.md)（英文）
- [从 UdonSharp 迁移](docs/migrating-from-udonsharp.md)（英文）
- [诊断代码](docs/diagnostics.md)（英文）
...
Udonite 由一个人开发和维护，包括每次 VRChat SDK 更新弄坏东西之后的修复。你的支持资助这项工作，以及在它之上构建的包（VRChat 世界开发中可复用的系统）。**目前还没有任何一个发布。** 支持者可以在每个包发布后立即使用。

[成为支持者](https://buy.polar.sh/polar_cl_fiMBXRqzbvm0c8qt6xqKKBGbaB0FOQmPMddgn06PRsC) · [查看正在考虑的包](docs/packages.md)（英文）
