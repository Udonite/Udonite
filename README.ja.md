...
`MonoBehaviour` を書いて保存するだけです。Udonite は Unity がスクリプトを再コンパイルしたときとシーンを保存したときにコンパイルし、各ビヘイビアと同じ GameObject に Udon プログラムを付けます。何かを拒否したとき以外は何も出力しません。Play モードで動くのは Udon プログラムそのものなので、エディタで見えているものがそのままアップロードされます。
